...
class CoordinationCalc():
    ...
    def get_neighbors(self, df, x, y, cur_time, validation):
        if validation == True:
            # Get all neighbors
            ...
            neighbors = df[(np.sqrt((df['x'] - x[0]) ** 2 + (df['y'] - y[0]) ** 2) <= self.NEIGHBORING_DISTANCE) &
                           (df['t_stamp'] == cur_time) &
                           (self.NEIGHBORING_DISTANCE - 50 < np.sqrt((df['x'] - x[0]) ** 2 + (df['y'] - y[0]) ** 2))]
    ...
    def calc_cos_of_angles(self, start_time, end_time, track, tracks01, df, validation):
        ...
        for i, cur_time in enumerate(range(start_time, end_time - self.SMOOTHING_VAR - 1)):
            ...
            neighbors = self.get_neighbors(df, x, y, cur_time, validation)
    ...
    def build_coordination_df(self, validation):
        ...
        for ind, track in enumerate(tracks01, start=0):
            ...
            cos_of_angles = self.calc_cos_of_angles(start_time, end_time, track, tracks01, df, validation)
            tmp_df = pd.DataFrame({'Track #': [ind], 't0': [start_time], 'cos_theta': [cos_of_angles]})
            self.coherence = pd.concat([self.coherence, tmp_df])

    # def build_coordination_df(self, tracks01, df):
    ...
    #         print('Track #{}/{}'.format(str(ind), len(tracks01)))
    #         t_stamp_array = np.asarray(track['t_stamp'])
    #         start_time = int(np.min(t_stamp_arraFy))
    #         end_time = int(np.max(t_stamp_array))
    #         cos_of_angles = self.calc_cos_of_angles(start_time, end_time, track, tracks01, df, False)
    #         tmp_df = pd.DataFrame({'Track #': [ind], 't0': [start_time], 'cos_theta': [cos_of_angles]})
    #         self.coherence = pd.concat([self.coherence, tmp_df])
...
if __name__ == '__main__':
    for i in range(1,13):
        coord_control = CoordinationCalc(5, 30,
                                         r"muscle-formation-diff/data/tracks_xml/0104/Experiment1_w1Widefield550_s{}_all_0104.xml".format(i))
        coord_control.build_coordination_df(False)
        coord_control.save_coordinationDF(r"coordination_df_s{}_0104.pkl".format(i))
