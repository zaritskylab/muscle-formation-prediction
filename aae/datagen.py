...
def aae_load_images(image_size):
    directory = "../../data/images/new_test"
    ...
    while True:
        ...
        for j in range(batch_size):
            if (i < len(file_list)):
                sample = file_list[i]
                i += 1
                image = cv2.imread(directory + "/" + sample,cv2.IMREAD_UNCHANGED)
                image = cv2.resize(image, (img_size, img_size), interpolation=cv2.INTER_AREA)
                image = image.astype("float32")
                image = (image - image.min()) / (image.max() - image.min())
...
def val_generator(directory, batch_size, img_size, validation_split):
    i = 0
    file_list = os.listdir(directory)
    shuffle(file_list)
    file_list = random.sample(file_list, int(np.floor(len(file_list) * validation_split)))
    while True:
        image_batch = []
